...
impl ReadConfig {
    ...
    fn read_byte(&self, f: &mut dyn io::Read) -> io::Result<u8> {
        let mut buffer: [u8; 1] = [0];
        f.read_exact(&mut buffer)?;
        Ok(buffer[0])
    }

    fn read_int(&self, f: &mut dyn io::Read) -> io::Result<i64> {
        if self.int_size == 0 {
    ...
    fn read_string(&self, f: &mut dyn io::Read) -> io::Result<String> {
    ...
    fn read_int_bool(&self, f: &mut dyn io::Read) -> io::Result<bool> {
        self.read_int(f).map(|v| v != 0)
    }

    fn read_string_bool(&self, f: &mut dyn io::Read) -> io::Result<bool> {
        self.read_string(f).map(|v| v == "true")
    }

    fn read_oid(&self, f: &mut dyn io::Read) -> io::Result<Oid> {
    ...
    fn read_offset(&self, f: &mut dyn io::Read) -> io::Result<Offset> {
